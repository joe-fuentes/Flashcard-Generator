    ...
    inquirer.prompt([
        ...
        if (cardType === "Basic Card") {
            ...
            inquirer.prompt([
                {
                    type: "input",
                    message: "Please type out the full fullText of your statement (remove cloze in next step).",
                    name: "fullText"
                },

                {
                    type: "input",
                    message: "Please type the portion of fullText you want to cloze, replacing it with '...'.",
                    name: "cloze"
                }

            ]).then(function (cardData) {            //once we have the users cloze data run this function

                var cardObj = {						//builds and object from the fullText and cloze info
                    type: "ClozeCard",
                    fullText: cardData.fullText,
                    cloze: cardData.cloze
                };
                if (cardObj.fullText.indexOf(cardObj.cloze) !== -1) {   //checking to make sure the Cloze matches some fullText in the statement
                    library.push(cardObj);							//push the new card into the array of cards
                    ...
                    console.log("Sorry, The cloze must match some word(s) in the fullText of your statement.");
...
function showCards () {
  ...
  if (count < library.length) {                     //if counter stays below the length of the library array
    ...
    if (library[count].front !== undefined) { //if card has a front then it is a Basic card
        ...
        console.log(colors.yellow("++++++++++++++++++++++++++++++++++++++++++++++++"));
        console.log("fullText: " + library[count].fullText); //grabs & shows card question
        console.log("------------------------------------------------");
